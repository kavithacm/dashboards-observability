...
const moveToMetricsHome = () => {
  cy.visit(
    `${Cypress.env('opensearchDashboards')}/app/observability-dashboards#/metrics_analytics/`
  );
  cy.wait(delay * 3);
};
...
describe('Has working breadcrumbs', () => {
    it('Redirect to correct page on breadcrumb click', () => {
      moveToMetricsHome();
      cy.get('.euiBreadcrumb[href="#/metrics_analytics"]').contains('Metrics analytics').click();
